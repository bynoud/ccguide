    ...
    render() {
        ...
        return <div>
                ...
                {rows.map(r => 
                    <div className="ChessRow" key={`CR${r}`}>
                        {cols.map(c => {
                            ...
                            const pieceTurn = (side === piece2side(this.state.curTurn)) &&
                                              ( !this.state.aiMove ||
                                                (this.state.human_first ? side === 'D' : side === 'T') );
                            //console.log("tile", tile, piece, this.state);
                            return <ChessTile key={tile} 
                                piece={piece} highligh={highligh}
                                clickable={highligh || (pieceTurn && isSameSide(piece, this.state.curTurn))}
                                clicked={this.chessTileClicked.bind(this, tile, piece)}
                                selected={this.state.selectedTile === tile}
                                focused={this.state.lastMoveFrom === tile || this.state.lastMoveTo === tile}
                            ></ChessTile>
                        }
                        )}
                    </div>
            ...
            <div className="BoardStatus">
                <p>AI to move: {this.state.aiMove ? "Allowed" : "None"}</p>
            ...
            <button onClick={this.reset.bind(this, false)}>Start. AI first</button>
            <button onClick={this.reset.bind(this, true)}>Start. Human first</button>
